...
START_TEST(sub_test_1) {
  s21_decimal number1 = {{4641631, 0, 0, 0}};
  s21_decimal number2 = {{314545, 0, 0, 0}};
  s21_decimal result = {};
  int add_res = s21_sub(number1, number2, &result);
  ck_assert_int_eq(add_res, 0);
  ck_assert_int_eq(result.bits[0], 4327086);
...
START_TEST(sub_test_2) {
  s21_decimal number1 = {{MAXBYTE, 563135135, 0, 0}};
  s21_decimal number2 = {{4000000000, 1357869, 0, 0}};
  s21_decimal result = {};
  int add_res = s21_sub(number1, number2, &result);
  ck_assert_int_eq(add_res, 0);
  ck_assert_int_eq(result.bits[0], 294967295);
  ck_assert_int_eq(result.bits[1], 561777266);
  ck_assert_int_eq(result.bits[2], 0);
  ck_assert_int_eq(result.bits[3], number1.bits[3]);
}
END_TEST
...
START_TEST(sub_test_3) {
  s21_decimal number1 = {{123456789, MAXBYTE, 987654321, 0}};
  s21_decimal number2 = {{89, MAXBYTE, 321, 0}};
  s21_decimal result = {};
  int add_res = s21_sub(number1, number2, &result);
  ck_assert_int_eq(add_res, 0);
  ck_assert_int_eq(result.bits[0], 123456700);
  ck_assert_int_eq(result.bits[1], 0);
  ck_assert_int_eq(result.bits[2], 987654000);
  ck_assert_int_eq(result.bits[3], number1.bits[3]);
}
END_TEST
...
static s21_decimal result[] = {
    ...
    {{0xDA380001, 0x5C68A936, 0x3E14F384,
...
START_TEST(sub_test_4) {
  for (size_t i = 0; i < sizeof(num1) / sizeof(s21_decimal); ++i) {
    s21_decimal tmp = {{0x000002FD, 0x00000000, 0x00000000, 0x80050000}};
...
START_TEST(sub_test_6) {
  for (size_t i = 0; i < sizeof(err_num1) / sizeof(s21_decimal); ++i) {
    s21_decimal tmp = {{0x000002FD, 0x00000000, 0x00000000, 0x80050000}};
    int ret = s21_sub(err_num2[i], err_num1[i], &tmp);
    if (tmp.bits[0] == 0) {
      tmp.bits[0] = 1;
    }
    ck_assert_int_eq(ret, err_result[i]);
...
Suite *sub_tests() {
  ...
  tcase_add_test(tcase, sub_test_6);
