...
void printPointer(char**str,PARAMETRS*newPrt,void*pointer);
void printG(char **str, PARAMETRS *newPtr, long double number);
//void decimalPart(char **s, char *dp, long double number,PARAMETRS *newPtr,bool flag);
...
/*--------------------------------------------------------*/
/* Нижеследующие функции реализованы с выделением динамической памяти*/
void *s21_to_upper(const char *str);
void *s21_to_lower(const char *str);
void *s21_insert(const char *src, const char *str, s21_size_t start_index);
void *s21_trim(const char *src, const char *trim_chars);
/*-------------------s21_sscanf functions---------------------*/
typedef struct {
    char specifier;
    int accepted;
    int error;
...
int s21_is_hex_letter(char c);
int s21_is_digit(char c);
char s21_lower(char c);
void skip_spaces(const char **str, const char **format);
int is_len_specifier(char c);
long int select_number_from_str(const char **format, SSCANF_PARAMS *param);
